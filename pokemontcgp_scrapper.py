...
import urllib3
import argparse
from constants import BASE_URL, FILENAME, START_ID, TYPE_MAPPING, SETS, SETS_CARDS_NO, PACKS, FULLART_RARITIES, CRAFTING_COST, RATE_BY_RARITY

def map_attack_cost(cost_elements):
  print(f"cost_elements: '{cost_elements}'.")
  cost_list = []
...
def fetch_and_parse_card(set_name, card_id):
  url = f"{BASE_URL}{set_name}/{card_id}"
  response = requests.get(url, verify=False)
  soup = BeautifulSoup(response.content, "html.parser")
  return extract_card_info(soup)

def process_sets(start_id, save_to_file=False, filename="cards.json"):
  cards = []
  for set_name in SETS:
    error_tracker = 0
    for i in range(start_id, SETS_CARDS_NO[set_name] + 1):
      try:
        card_info = fetch_and_parse_card(set_name, i)
...
###############################################################################################
def iterate_per_set(set_name, start_id):
  for i in range(start_id, SETS_CARDS_NO[set_name] + 1):
    try:
      card_info = fetch_and_parse_card(set_name, i)
      for key, value in card_info.items():
...
def iterate_all_sets():
  for set_name in SETS:
    iterate_per_set(set_name, START_ID)
...
#def convert_cards_to_json(start_id, filename):
#  cards = []
#  error_tracker = 0
#  for set_name in SETS:
#    for i in range(start_id, SETS_CARDS_NO[set_name] + 1):
...
def main():
  ...
  init_time = time.time()
  start_id = START_ID
#  iterate_all_sets() # NOT NEEDED - Just for printing
#  convert_cards_to_json(start_id, filename)

  parser = argparse.ArgumentParser(description="Scrape Pokémon TCG Pocket cards and optionally save to JSON.")
  parser.add_argument('--save', dest='save_to_file', action='store_true', help='Save output to JSON file (default: True)')
  parser.add_argument('--no-save', dest='save_to_file', action='store_false', help='Do not save output to file')
  parser.set_defaults(save_to_file=True)
  parser.add_argument('--filename', type=str, default=FILENAME, help='Filename to save JSON data (default from constants.py)')

  args = parser.parse_args()
  process_sets(start_id, save_to_file=args.save_to_file, filename=args.filename)

  end_time = time.time()
  print(f"Finished downloading cards to {args.filename}, total time: {end_time - init_time} seconds.")
